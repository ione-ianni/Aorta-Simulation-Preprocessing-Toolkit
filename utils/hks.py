...
import argparse
from trimesh_hks import load_off, save_off
...
if __name__ == '__main__':
    ...
    mesh = pv.read(opt.input)
    # Extract nodal coordinates and connectivity matrix
    VPos = np.array(mesh.points)
    ITris = np.array(mesh.faces.reshape(-1, 4)[:, 1:4])
    # Create VColors variable initialized with zeros
    VColors = np.zeros((len(VPos), 3))
    ...
    # Save the mesh in a vtp file with the HKS
    # Create a new point array to associate each node ID with the corresponding weight
    '''HeatKernelSignature = np.zeros(mesh.n_points)
    # Set the weights for the corresponding node IDs
    node_ids = np.arange(1, len(VPos) + 1)
    # Set the weights for the corresponding node IDs
    HeatKernelSignature[node_ids - 1] = hks[:, 0]  # Adjust indexing to start from 0
    mesh['HeatKernelSignature'] = HeatKernelSignature
    mesh.save(opt.output)'''
    # Add the heat kernel signature as point data scalar to the mesh
    mesh['hks'] = hks[:, 0]

    # Save the mesh with the heat kernel signature as a VTP file
    mesh.save(opt.output)
